...
const resolvers = {
  Query: {
    ...
    getFeedbacks: async function (parents, args, context) {
      ...
      if (authStaus) {
        ...
        if (getUserFeedBacks) {
          const allFeedbacksForCurrentUser = await Feedback.find({
            _id: { $in: getUserFeedBacks.feedback_recieved },
          });
  ...
  Mutation: {
    ...
    deleteFeedback: async function (parent, args, { user, authStaus }) {
      ...
      if (authStaus) {
        ...
        if (removeFeedback) {
          ...
          if (currentUser) {
            currentUser.feedback_recieved.splice(
              currentUser.feedback_recieved.indexOf(id),
              1
            );
            currentUser.save();
            return true;
    ...
    AuthenticateFacebookUser: async function (
      parent,
      args,
      { user, authStaus }
    ) {
      const { userID, accessToken, name, email, short_name, graphDomain } =
        args;
    ...
    AuthenticateGoogleUser: async function (parent, args, { user, authStaus }) {
      const { googleId, accessToken, profileObj } = args;
      const userDoc = await User.findOne({ userID: googleId });
      // if user not found create new one
      if (!userDoc) {
        const tempUser = await new User({
          userID: googleId,
          ...
          source: 'google',
          short_name: profileObj.givenName,
